...
from src.detector import (detect_bruteforce, detect_password_spraying,
                             detect_impossible_travel, run_all_detections)
...
class TestMapMitre:
    ...
    def test_unknown(self):
        assert map_mitre("T9999") == "T9999"  # returns ID if not in mapping
...
class TestDetectBruteforce:
    ...
    def _make_logs(ip, count, minutes_apart=0.1):
        ...
        return [{"ip": ip, "user": "admin", "status": "FAILED",
...
class TestRunAllDetections:
    def test_deduplication(self, brute_force_logs):
        ...
        assert len(rule_ids) == rule_ids.count(rule_ids[0])  # no duplicates
